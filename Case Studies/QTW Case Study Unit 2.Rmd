...
Weighted KNN Using all Seven MAC Addresses
```{r}
...
train_weightedKNN = kknn(posXY ~ ., train = leavout_train %>% select(-c("posX","posY")), test = leavout_test %>% select(-c("posX","posY")), distance = 1, k = 3, kernel = "inv")


knnPredict <- train_weightedKNN$fitted.values
temp_res = data.frame(predictions = knnPredict, TrueX = leavout_test$posX, TrueY = leavout_test$posY)
...
orig_mac_residuals = data.frame(predictions = knnPredict, PredX = as.numeric(unlist(split_x)), PredY = as.numeric(unlist(split_y)),  TrueX = as.numeric(leavout_test$posX), TrueY = as.numeric(leavout_test$posY))
...
Final_results[4,3] = RMSE



```

Building a Weighted KNN to predict test data

```{r}

testing = online_pivot%>% select(-c("posX","posY","posXY","posZ","orientation","type","time"))
 

#Distance is minkowski =1 
#Using the parameter of K = 3 from earlier work
weightedKNN = kknn(posXY ~ ., train = training %>% select(-c("posX","posY")), test = testing, distance = 1, k = 3, kernel = "inv")
...
print(RMSE)
```
Graphic output - Given teh above has run in env
```{r}
#Compare accuracies and Ks of first three models
plot(knnFit)
