...
Final_results = data.frame("Model_Type" = c("Original Six Mac Addresses", 
                                            ...
                                            "All Seven Mac Addresses", "Weighted KNN Model"), 
                           "K" = rep(0,4), "Accuracy"= rep(0,4), "Time" = rep(0,4), "RMSE" = rep(0,4))
...
knnFit <- train(posXY ~ ., data = original_six_mac_offline %>% 
                  select(-c("posX","posY")), method = "knn", trControl = train.control, tuneGrid = knn.grid)
...
orig_mac_residuals = data.frame(predictions = knnPredict, 
                                PredX = as.numeric(unlist(split_x)), PredY = as.numeric(unlist(split_y)), 
